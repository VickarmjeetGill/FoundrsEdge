...
import { Calendar, Building2, BookOpen, Trophy, Video, Users, Star, LogOut, Plus, CheckCircle, X, Pencil, Trash2, ChevronDown, ChevronUp, LayoutDashboard, ClipboardList, Tag, Flag, Upload, Milestone, Activity } from 'lucide-react';
...
function ResourcesSection({ onSuccess, setConfirmModal }: { onSuccess: (msg: string) => void; setConfirmModal: any }) {
  ...
  async function loadResources() {
    try {
      ...
      if (res.ok) {
        const data = await res.json();
        setResourcesList(Array.isArray(data) ? data : (data.resources || []));
      }
    } catch (err) {
      console.error(err);
    }
